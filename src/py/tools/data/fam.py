...
def get_species_tree(datadir, subst_model=None, method="true"):
    ...
    if (subst_model != None):
        return os.path.join(get_species_dir(datadir),
                            method + "." + subst_model + ".speciesTree.newick")
    else:
...
def build_gene_tree_path(datadir, subst_model, family, method):
    if (method == "true" or subst_model == "true"):
        ...
        return os.path.join(get_gene_tree_dir(datadir, family),
                            method + "." + subst_model + ".geneTree.newick")
...
def build_likelihood_path(datadir, subst_model, family, method):
    return os.path.join(get_likelihoods_dir(datadir, family),
                        method + "." + subst_model + ".geneTree.newick")
...
def get_fastme_distances(datadir, family, subst_model):
    return os.path.join(get_family_misc_dir(datadir, family),
                        "fastme_matrix." + subst_model + ".txt")
...
def get_successful_runs(datadir):
    ...
    for directory in directories_to_check:
        for tree in os.listdir(directory):
            ...
            if (not "raxmlMultiple" in run_name and not run_name.startswith(
                    "mrbayes") and not run_name.startswith("ccp")):
...
def postprocess_datadir(datadir):
    ...
    if (os.path.isfile(get_species_tree(datadir))):
        convert_to_phyldog_species_tree(get_species_tree(datadir),
                                        get_phyldog_species_tree(datadir))
        # alignments
    for family in get_families_list(datadir):
        family_dir = get_family_path(datadir, family)
        utils.relative_symlink(get_alignment(datadir, family),
                               os.path.join(datadir, "alignments", family + ".fasta"))
        convert_phyldog_to_treerecs_mapping(get_mappings(datadir, family),
                                            get_treerecs_mappings(datadir, family))
