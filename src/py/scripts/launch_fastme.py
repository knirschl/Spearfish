    ...
    with open(scheduler_commands_file, "w") as writer:
        for family in fam.get_families_list(datadir):
            ...
            for miscfile in os.listdir(misc_dir):
                ...
                command.append("-o")
                command.append(os.path.join(misc_dir, miscfile.replace(output_prefix,
                                                                       output_prefix + "fastme.").replace(
                    "matrix.phy", "geneTree.newick")))
                writer.write(" ".join(command) + "\n")
    return scheduler_commands_file
...
def run_fastme_on_families(datadir, subst_model, is_dna, algo, use_spr, only_mat, cores):
    ...
    scheduler_commands_file = generate_scheduler_commands_file(datadir, subst_model, is_dna, algo,
                                                               use_spr, only_mat, output_dir)
    start = time.time()
    utils.run_with_scheduler(paths.fastme_exec, scheduler_commands_file,
...
def run_fastme_on_families_matrices(datadir, mat_prefix, algo, use_spr, cores):
    ...
    shutil.rmtree(output_dir, True)
    os.makedirs(output_dir)
    scheduler_commands_file = generate_scheduler_commands_file_matrices(datadir, mat_prefix, algo,
                                                                        use_spr, output_dir)
    start = time.time()
    utils.run_with_scheduler(paths.fastme_exec, scheduler_commands_file,
                             paths.mpi_scheduler_heuristic, cores, output_dir, "logs.txt")
...
if (__name__ == "__main__"):
    ...
    if len(sys.argv) < max_args_number:
        print("Syntax error: python " + os.path.basename(
            __file__) + "  datadir subst_model is_dna cores.")
