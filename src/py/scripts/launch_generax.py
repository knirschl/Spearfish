...
def get_generax_command(generax_families_file, species_tree, strategy, rec_model,
                        additional_arguments, output_dir, mode, cores):
...
def run_generax(datadir, subst_model, strategy, rec_model, species_tree, generax_families_file,
                mode, cores, resultsdir, additional_arguments=""):
    command = get_generax_command(generax_families_file, species_tree, strategy, rec_model,
                                  additional_arguments, resultsdir, mode, cores)
    # print(command)
...
def eval_and_pick(datadir, results_dir):
    ...
    for family in os.listdir(results_dir):
        true_family, tree = family.split(eval_results_delim)
...
def run(datadir, subst_model, strategy, species_tree, starting_tree, cores, additional_arguments,
        resultsdir, do_extract=True):
    run_name = utils.getAndDelete("--run", additional_arguments, None)
    if (None == run_name):
        run_name = get_run_name(species_tree, starting_tree, subst_model, strategy,
    ...
    empty = False
    # if (strategy == "EVAL"):
    empty = build_generax_families_file_eval(datadir, subst_model, generax_families_file,
                                             tree_prefix=starting_tree)
    # else:
    #  build_generax_families_file(datadir, starting_tree, subst_model, generax_families_file)
    start = time.time()
    if (not empty):
        run_generax(datadir, subst_model, strategy, rec_model, species_tree, generax_families_file,
                    mode, cores, resultsdir, additional_arguments)
...
def launch(datadir, subst_model, strategy, species_tree, starting_tree, cluster, cores,
           additional_arguments):
    command = ["python3"]
    command.extend(sys.argv)
    command.append("--exprun")
    dataset = os.path.basename(datadir)
    resultsdir = os.path.join("GeneRax", dataset,
                              strategy + "_" + species_tree + "_start_" + starting_tree, "run")
...
if (__name__ == "__main__"):
    ...
    if (len(sys.argv) < min_args_number):
        print("Syntax error: python " + os.path.basename(
            __file__) + "  dataset species_tree gene_trees subst_model strategy cluster cores [additional paremeters].\n  ")
        sys.exit(1)
    ...
    if (is_run):
        run(datadir, subst_model, strategy, species_tree, starting_tree, cores,
            additional_arguments, resultsdir)
    else:
        launch(datadir, subst_model, strategy, species_tree, starting_tree, cluster, cores,
               additional_arguments)
